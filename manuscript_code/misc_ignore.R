...
# =============================================================================
# UPTIME OVER TIME (INDIVIDUAL UNITS + ROLLING AVERAGE)
# =============================================================================

create_uptime_timeseries <- function(uptime_data) {
  
  # Calculate 7-day rolling average uptime for each individual unit
  individual_rolling <- uptime_data %>%
    arrange(id, date) %>%
    group_by(id, method) %>%
    mutate(
  ...
  # Calculate system-wide rolling averages
  system_rolling <- individual_rolling %>%
  ...
  uptime_timeseries <- ggplot() +
    # Individual unit lines (thin, transparent)
    geom_line(data = individual_rolling, 
              aes(x = date, y = uptime_7day, group = id, color = method), 
              alpha = 0.3, size = 0.3) +
    # System average lines (thick)
    geom_line(data = system_rolling, 
              aes(x = date, y = mean_7day, color = method), 
              size = 1.5) +
...
# Create combined layout: barplot (3/4 width) and boxplot (1/4 width) on top, timeseries on bottom
library(patchwork)

uptime_combined <- (uptime_barplot + uptime_boxplot + plot_layout(widths = c(3, 1))) / uptime_timeseries + 
...
cat("- system_uptime_combined.png (barplot + boxplot top, timeseries bottom)\n")