...
HFauto_fluxestimates$id<-HFauto_fluxestimates$chamber
...
met_data$Time <- as.POSIXct(met_data$Time)


library(fuzzyjoin)

# Perform fuzzy join based on closest datetime with a larger time difference limit (60 mins)
merged_data_with_met <- difference_left_join(merged_data, met_data, 
...
ggplot(merged_data_with_met, aes(x = soiltemp_10cm_HF, y = fluxL_umolm2sec, color=method)) +
...
p2<-ggplot(data=merged_data_with_met, aes(x=ymd_hms(hour_of_obs), y=soiltemp_10cm_HF))+
