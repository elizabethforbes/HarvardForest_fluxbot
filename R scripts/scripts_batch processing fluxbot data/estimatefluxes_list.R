...
calculate_regressions_custom_intervals_list <- function(time_series_list, custom_intervals_df, 
                                                   method, volume, area) {
  results_list <- list() # list to store final merged data frames
  
  for (i in seq_along(time_series_list)) {
    time_series_df <- time_series_list[[i]]  # Get the current dataframe from the list
  ...
  for (i in seq_len(nrow(custom_intervals_df))) {
    ...
    # Perform further calculations only if interval data is not empty after removing specific value
    if (length(interval_data) > 0) {
      # continue with your calculations...
    } else {
      cat("Skipping interval due to specific value:", i, "\n")
      next  # Skip to the next iteration
    }
  ...
  # store the results df in a list with the name of the input df
  results_list[[paste0("result_", names(time_series_list)[i])]] <- results_df
  }
  
  # Return the results in a list of dfs
  return(results_list)
}
