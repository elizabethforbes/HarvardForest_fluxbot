...
library(fuzzyjoin) # for merging datasets by timestamp
library(smplot2) # stats on plots
library(gtsummary) # pub-ready tables
```
...
```{r}
# fluxbot estimates:
HF_fluxestimates <- read.csv("HarvardForest_fluxestimates_fall2023_withstartendconcens.csv", header = TRUE)
...
# change 'healthy' to 'stand 1', and 'unhealthy' to 'stand 2':
merged_data_with_met <- merged_data_with_met %>% 
  mutate(stand = case_when(
    stand == "healthy" ~ "stand 1",
    stand == "unhealthy" ~ "stand 2"
  ))
...
## note: add line to remove HF array data on low-temp days
...
### Table 1: summary statistics across arrays for subset of data where we have matching fluxes

```{r}
#| echo: FALSE

# overall mean, median, 95th percentiles, SE for arrays
# daytime mean, median, 95th percentiles, nighttime mean, SEs

# night: hours 15-20 (highest fluxes according to figure 6)
# day: hours 5-10 (lowest fluxes according to figure 6)

merged_data_with_met_filter <- merged_data_with_met_filter %>% 
  mutate(timeofday = case_when(
    hour >= 15 & hour <= 20 ~ "evening",
    hour >= 5 & hour <= 10 ~ "morning",
    TRUE ~ "mid"
  ))
  
table_data <- merged_data_with_met_filter %>% 
  mutate(stand = case_when(
    stand == "healthy" ~ "stand 1",
    stand == "unhealthy" ~ "stand 2"
  ))
...
calc_summary_stats <- function(x) {
  data.frame(
    ...
    SE = round(sd(x, na.rm = TRUE) / sqrt(length(x[!is.na(x)])), 2),
    N = length(x[!is.na(x)])
  )
}


# Calculate summary statistics by location and time period
summary_table <- table_data %>%
  filter(timeofday == c("morning", "evening")) %>% 
  group_by(method, stand, timeofday) %>%
  summarise(
    stats = list(calc_summary_stats(fluxL_umolm2sec)),
    .groups = "drop"
  ) %>%
  unnest(stats)
...
formatted_table <- summary_table %>%
  arrange(method, stand, timeofday)

# Create publication-ready table
pub_table <- formatted_table %>%
  kable(
    col.names = c("Array", "Stand", "Time of day", 
                  "Mean", "Median", "95th Percentile", "SE", "N"),
    caption = "Summary Statistics: Flux by Array type, Location, and Approx. Min/Max Time of Day",
  ...
  pack_rows('Autochamber array:', 1, 4) %>%
  pack_rows('Fluxbot array:', 5, 8) %>%
  add_header_above(c(' ' = 3, 'Summary Statistics' = 4, ' ' = 1)) %>%
  footnote(
    general = c('SE = Standard Error', 'P95 = 95th Percentile', 'N = number of flux obs.',
                "morning = 5am - 10am", "evening = 3pm - 8pm"),
    general_title = 'Note:'
  )
...
# Save tables in journal-appropriate formats
library(webshot2)

# Save as high-resolution image (PNG) - good for most journals
save_kable(pub_table, 
           file = "summary_statistics_table.png", 
           zoom = 2)  # High resolution

# Save as PDF (vector format) - best quality for print journals
save_kable(pub_table, 
           file = "summary_statistics_table.pdf",
           zoom = 1.5)
  arrange(location, time_period)

```

### General summary statistics: chamber-level analyses, means and medians, IQRs

```{r}
tbl <- merged_data_with_met %>% 
  group_by(id) %>% 
  summarise_at(vars(fluxL_umolm2sec),
...
### Comparison of high-baseline flux estimates from fluxbots to autochamber equivalents

```{r}
#| echo: FALSE
# Filter for flux estimates calculated from high (>900ish) concentrations
# Determine if flux estimate is comparable to HF array estimates
# Performance under high RH?

# identify those fluxes captured by the fluxbots with starting concentrations of CO2 >= 850ppm
HF_fluxestimates_highstart <- HF_fluxestimates %>% 
  filter(starting_concen >= 850)
# 516 total estimates with a high start of over 849ppm

# select those of these high concentration fluxes that fall within the filtered fluxbot dataset:
HF_fluxestimates_highstart2 <- HF_fluxestimates_highstart %>% 
  inner_join(HF_fluxestimates_filtered)
# 489 total flux estimates with a high start over 849ppm

# select those HFautochamber (filtered) data from the timestamps in HF high start:
matching_auto <- filter(HFauto_fluxestimates_filtered_avg, 
                        hour_of_obs %in% HF_fluxestimates_highstart2$hour_of_obs)
# 3146 observations (out of 6726) that match the hour_of_obs for our high-start fluxes (across the whole of both arrays).

# merge data, plot:
HF_fluxestimates_highstart3 <- HF_fluxestimates_highstart2 %>% 
  select(id, hour_of_obs, date, hour, fluxL_umolm2sec, stand, method, starting_concen, ending_concen)

matching_auto2 <- matching_auto %>% 
  # add cols for starting and ending concentration, fill with NA
  mutate(starting_concen = NA,
         ending_concen = NA)

highinitial_fluxes_merged <- rbind(HF_fluxestimates_highstart3, matching_auto2)

###################
# alt version where we take the average for each hour for the autochambers since there are multiple, then merge by timestamp with the high start dataset from the bots:
highinitial_auto_avg <- matching_auto2 %>% 
  group_by(hour_of_obs) %>% 
  summarise(fluxL_umolm2sec_auto = mean(fluxL_umolm2sec))
highinitial_fluxes_avg <- HF_fluxestimates_highstart3 %>% 
  group_by(hour_of_obs) %>% 
  summarise(fluxL_umolm2sec_fluxbot = mean(fluxL_umolm2sec))
            
highinitial_fluxes_merged_avg <- highinitial_fluxes_avg %>% 
  left_join(highinitial_auto_avg, by = "hour_of_obs")

###################
# alt alt version where we try to retain the hour, stand meta-data:
highinitial_auto_avg2 <- matching_auto2 %>% 
  select(!c(date, hour, method, id)) %>% 
  group_by(hour_of_obs, stand) %>% 
  summarise(fluxL_umolm2sec_auto = mean(fluxL_umolm2sec)) %>% 
  mutate(method = "autochamber")

highinitial_merged_multiple <- HF_fluxestimates_highstart3 %>% 
  left_join(highinitial_auto_avg2, by = c("hour_of_obs", "stand")) %>% 
  rename(fluxL_umolm2sec_fluxbot = fluxL_umolm2sec,
         starting_conc_fluxbot = starting_concen,
         ending_conc_fluxbot = ending_concen) %>% 
  select(!c(method.x, method.y))

```

Plot the high-starting-concentration fluxbot flux data, paired with the nearest-in-time autochamber data, summarized in various iterations:

```{r}
p = palette(hcl.colors(2, "Batlow"))

labels <- as_labeller(
  c("healthy" = "stand 1", 
    "unhealthy" = "stand 2"))

highinitial_fluxes_merged %>% 
  ggplot(aes(x = method, y = fluxL_umolm2sec))+
  geom_violin(aes(fill = method, alpha = .75))+
  ...
  ylab(expression(paste(CO[2], " Flux, ", mu, "mol ", m^-2, s^-1)))+
  theme(legend.position = "none")+
  facet_wrap(~stand, labeller = labels)
```

Linear comparison of the averaged-by-hour data:

```{r}
# Fit the linear model to the full data
# model <- lm(fluxL_umolm2sec_auto ~ fluxL_umolm2sec_fluxbot, 
#             data = highinitial_merged_multiple)
# # Extract the slope, intercept, and R-squared
# slope <- coef(model)[2]
# intercept <- coef(model)[1]
# r_squared <- summary(model)$r.squared
...
# Plot the comparison of the two systems (autochamber mean'd):
highinitial_merged_multiple %>% 
  ggplot(aes(x = fluxL_umolm2sec_auto, y = fluxL_umolm2sec_fluxbot))+
  geom_point(alpha=0.3)+
  labs(x = "Autochamber estimates (mean by hour of observation)",
       y = "Fluxbot estimates, high initial starting concentration")+
  theme_classic()+
  geom_smooth(method = "lm", color = "#5A82C9") +  # Linear regression line
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") + # Add a 1:1 line
  # annotate("text", x = 4.5, y = Inf, label = equation_label, 
           # hjust = 1, vjust = 1.5, size = 5, color = "#4C72B0") + # Add equation, r2
  xlim(0, 6) + ylim(0, 6)+
  facet_wrap(~stand, labeller = labels)
...
# likely a rainfall event before/on the 21st of October 2023, as there is a high number of high-initial-concentration flux estimates on that day compared to the others. They cluster on both sides of the 1:1 line that day.
...
highinitial_merged_multiple %>% 
  ...
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") + # Add a 1:1 line
  facet_wrap(~id)
# bot 100, 22, 24, 110 underestimated fluxes regularly compared to the (averaged) autochamber estimates; these differences could be due to differences in individual bots, or differences in their microsites compared to the averaged hour-and-stand data from the autochamber arrays, or... 
# bot 114 and 102 had a greater number of fluxes with high initial concentrations than all the others, though these scatter around the 1:1 line in relation to the average hour-of-obs-and-by-stand fluxes from the autochambers.
...
highinitial_merged_multiple %>% 
  ...
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") + # Add a 1:1 line
  facet_wrap(~hour)
# generally fluxbots underestimate compared to the (averaged by stand, hour of obs) fluxbot arrays for those observations where the initial concentration of CO2 was elevated. however, these are pretty evenly scattered around the 1:1 no matter the time of day (hour).

# by observation of the visuals alone, it does not seem likely that there is a systemic overestimation of flux when bots start at an elevated concentration of CO2 for an observation interval.
```

```{r}
# by starting concentration:
highinitial_merged_multiple %>% 
  ggplot(aes(x = starting_conc_fluxbot, y = fluxL_umolm2sec_fluxbot))+
  geom_point(alpha=0.3, color = "darkblue")+
  labs(x = "starting concentration of carbon dioxide (ppm)",
       y = "Fluxbot estimates, high initial starting concentration")+
  theme_classic()
  # geom_abline(intercept = 0, slope = 1, linetype = "dashed")

# there does not seem to be a relationship between starting concentration and resultant flux estimate
```

Correlation between starting concentration and flux estimate:

```{r}
# Shapiro-Wilk normality test for flux estimates (high initial concentration)
shapiro.test(highinitial_merged_multiple$fluxL_umolm2sec_fluxbot) 
# W = 0.89744, p-value < 2.2e-16
ggqqplot(highinitial_merged_multiple$fluxL_umolm2sec_fluxbot,
         ylab = "fluxbot estimate")

# Shapiro-Wilk normality test for autochamber estimates (averaged by hour, stand)
shapiro.test(highinitial_merged_multiple$fluxL_umolm2sec_auto)
# W = 0.9227, p-value = 8.359e-14
ggqqplot(highinitial_merged_multiple$fluxL_umolm2sec_auto,
         ylab = "average autochamber estimate")

# Shapiro-Wilk normality test for initial concentration
shapiro.test(highinitial_merged_multiple$starting_conc_fluxbot)
# W = 0.86825, p-value < 2.2e-16
ggqqplot(highinitial_merged_multiple$starting_conc_fluxbot,
         ylab = "fluxbot starting concentration")

# Spearman's correlation: between the two flux estimates
cor_1 <-cor.test(highinitial_merged_multiple$fluxL_umolm2sec_fluxbot,
                 highinitial_merged_multiple$fluxL_umolm2sec_auto,  
                 method = "spearman")
cor_1
# S = 9200746, p-value = 1.525e-06
...
Plot the correlation between average autochamber estimate, and fluxbot estimate:

```{r}
highinitial_merged_multiple %>% 
  ggplot(mapping = aes(x = fluxL_umolm2sec_fluxbot, y = fluxL_umolm2sec_auto)) +
  geom_point(shape = 21, fill = "#0f993d", color = "white", size = 2, alpha = 0.5) +
  sm_statCorr(
    color = "darkgreen", corr_method = "spearman",
    linetype = "dashed"
  )+
  labs(x = expression(paste("Fluxbot estimate, ", mu, "mol ", CO[2], " ", m^-2, s^-1)),
       y = expression(paste("Average autochamber estimate, ", mu, "mol ", CO[2], " ", m^-2, s^-1)))
```

Plot the correlation between fluxbot estimate and initial starting concentration:

```{r}
highinitial_merged_multiple %>% 
  ggplot(mapping = aes(x = fluxL_umolm2sec_fluxbot, y = starting_conc_fluxbot)) +
  geom_point(shape = 21, fill = "#0f993d", color = "white", size = 2, alpha = 0.5) +
  sm_statCorr(
    color = "darkgreen", corr_method = "spearman",
    linetype = "dashed"
  ...
  labs(x = expression(paste("Fluxbot estimate, ", mu, "mol ", CO[2], " ", m^-2, s^-1)),
       y = expression(paste("Initial concentration of ", CO[2], " (ppm)")))
```
